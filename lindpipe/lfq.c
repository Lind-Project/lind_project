...
void safe_free(struct lfq_ctx *ctx, struct lfq_node * lfn) {
	if (lfn->can_free && !inHP(ctx,lfn)) {
		...
		if (CAS(&ctx->is_freeing, 0, 1)) {
			...
			tc_free(lfn);    // we got the lock; actually free
			ctx->is_freeing = false;
			smb();
...
int lfq_init(struct lfq_ctx *ctx, int max_consume_thread) {
	struct lfq_node * tmpnode = tc_calloc(1,sizeof(struct lfq_node));
	if (!tmpnode) 
		return -errno;
		
	struct lfq_node * free_pool_node = tc_calloc(1,sizeof(struct lfq_node));
	if (!free_pool_node) 
		return -errno;
	...
	ctx->HP = tc_calloc(max_consume_thread,sizeof(struct lfq_node));
	ctx->tid_map = tc_calloc(max_consume_thread,sizeof(struct lfq_node));
...
int lfq_clean(struct lfq_ctx *ctx){
	...
	if ( ctx->fph && ctx->fpt ) {
		...
		if ( ctx->fph != ctx->fpt )
			return -1;
		tc_free((void *)ctx->fpt); // free the empty node
		ctx->fph=ctx->fpt=0;
	}
	if ( !ctx->fph && !ctx->fpt ) {
		tc_free((void *)ctx->HP);
		tc_free((void *)ctx->tid_map);
		memset(ctx,0,sizeof(struct lfq_ctx));
	} else
		return -1;
...
int lfq_enqueue(struct lfq_ctx *ctx, void * data) {
	struct lfq_node * insert_node = tc_calloc(1,sizeof(struct lfq_node));
	if (!insert_node)
		return -errno;
