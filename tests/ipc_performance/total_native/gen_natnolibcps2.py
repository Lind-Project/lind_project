...
def execute_script(write_buffer_size, read_buffer_size):
    _ = subprocess.call(
        ["/bin/bash", "psnolibc32var2.sh", write_buffer_size, read_buffer_size], cwd="./scripts/"
    )


run_times = {}

for size in range(4, 17, 4):
    ...
    with open(f"data/natnolibc_{args.write_buffer}_{args.read_buffer}.json", "w") as fp:
