...
for i in range(0, len(native_report), num_stats):
    ...
    copy_func = float(native_report[i + 5]) + float(native_report[i + 6]) + float(native_report[i + 7])
...
for key in native_data:
    data = [[native_data[key][i][j] for i in range(len(native_data[key]))] for j in range(len(native_data[key][0]))]
...
for key in lind_data:
    data = [[lind_data[key][i][j] for i in range(len(lind_data[key]))] for j in range(len(lind_data[key][0]))]
    data = [sum(stat) / len(stat) for stat in data]
