...
parser = argparse.ArgumentParser(description="Script to benchmark piping 16GB varying buffersize in lind")
...
for size in range(4, 17, 2):
    ...
    for _ in range(args.count):
        output = Popen(["lind", "-t", "/bin/bash", "ps16var2", write_buffer_size, read_buffer_size], stdout=PIPE, stderr=PIPE)
