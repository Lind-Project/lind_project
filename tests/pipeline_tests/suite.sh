...
cd /home/lind/lind_project/tests/pipeline_tests

lindfs deltree "/pipeline_tests/" &> /dev/null
...
for var in "$@"; do
    case "$var" in
        ...
        -*)
            echo "Invalid command line argument $var, usage: ./suite.sh nondets.txt -d bashdets.txt (-v)"
            exit;;
        *)
