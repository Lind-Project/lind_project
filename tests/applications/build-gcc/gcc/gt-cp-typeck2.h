...
gt_pch_nx_pending_abstract_type (void *x_p)
{
  struct pending_abstract_type * x = (struct pending_abstract_type *)x_p;
  struct pending_abstract_type * xlimit = x;
  while (gt_pch_note_object (xlimit, xlimit, gt_pch_p_21pending_abstract_type, gt_ggc_e_21pending_abstract_type))
   xlimit = ((*xlimit).next);
  while (x != xlimit)
    {
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_P21pending_abstract_type4htab, gt_e_P21pending_abstract_type4htab))
    {
      if ((*x).entries != NULL) {
        ...
        gt_pch_note_object ((*x).entries, x, gt_pch_p_P21pending_abstract_type4htab, gt_types_enum_last);
      }
    }
}
...
{
  struct pending_abstract_type * const x ATTRIBUTE_UNUSED = (struct pending_abstract_type *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).decl), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).type), cookie);
...
const struct ggc_root_tab gt_ggc_r_gt_cp_typeck2_h[] = {
  {
