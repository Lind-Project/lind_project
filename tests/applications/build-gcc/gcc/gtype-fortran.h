...
{
  struct module_htab_entry * const x = (struct module_htab_entry *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_17module_htab_entry, gt_ggc_e_17module_htab_entry))
    {
      gt_pch_n_S ((*x).name);
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_P17module_htab_entry4htab, gt_e_P17module_htab_entry4htab))
    {
      if ((*x).entries != NULL) {
        ...
        gt_pch_note_object ((*x).entries, x, gt_pch_p_P17module_htab_entry4htab, gt_types_enum_last);
      }
    }
}
...
{
  struct module_htab_entry * const x ATTRIBUTE_UNUSED = (struct module_htab_entry *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).name), cookie);
  if ((void *)(x) == this_obj)
