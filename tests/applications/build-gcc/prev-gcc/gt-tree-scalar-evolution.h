...
{
  struct scev_info_str * const x = (struct scev_info_str *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_13scev_info_str, gt_ggc_e_13scev_info_str))
    {
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_P13scev_info_str4htab, gt_e_P13scev_info_str4htab))
    {
      if ((*x).entries != NULL) {
        ...
        gt_pch_note_object ((*x).entries, x, gt_pch_p_P13scev_info_str4htab, gt_types_enum_last);
      }
    }
}
...
{
  struct scev_info_str * const x ATTRIBUTE_UNUSED = (struct scev_info_str *)x_p;
  if ((void *)(x) == this_obj)
...
const struct ggc_root_tab gt_ggc_r_gt_tree_scalar_evolution_h[] = {
  {
